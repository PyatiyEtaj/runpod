...
if [ -d "$DATASET_VENV" ]; then
  ...
  python -m pip install "huggingface-hub>=0.28.1,<1.0" "hf_transfer>=0.1.8,<0.2.0" "rich>=13.8.0" "numpy<2.0.0" "pillow<12.0" "scipy<1.12" "kornia>=0.7,<0.9" "opencv-python-headless>=4.8,<5.0" "realesrgan>=0.3.0,<0.4.0" -c "$PROJECT_DIR/constraints/dataset-cu124.txt"
  python -m pip check || true
  deactivate
fi
